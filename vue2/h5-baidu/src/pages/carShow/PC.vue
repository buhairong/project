<template>
  <div id="wrapper-pc">
    ...
    <transition name="trans-search">
      <SearchPanel v-show="search" :current-city="selectedCity" :is-pc="true" ref="searchPanel"
                   :city="config.hotCities" :code="selectedCityCode"/>
    </transition>
    <ResPanel v-show="showResource" :data="res" ref="resPanel" :panel="resPanel" :is-pc="true"/>
    <Car/>
  </div>
</template>
...
import {
  addImgMarker,
  addMapEvent, CLUSTER_SIZE,
  CLUSTER_SIZE_LARGE,
  getDistanceByZoom,
  initMap, MARKER_NIO_SIZE,
  MARKER_SIZE,
  MARKER_SIZE_LARGE
} from '@/map'
...
import {getPowerDetail, getCitySummary, getPowerAround, getPoiSummary} from "@/api";
...
export default {
  ...
  mounted() {
    ...
    PubSub.subscribe('selectRes', function (msg, res) {
      ...
      getPowerAround(res.lat, res.lng, getDistanceByZoom(_this.map.zoom), _this, function (r) {
        ...
        getPoiSummary(_this, res.lat, res.lng, _this.config.distance, function (data) {
          _this.panel.data = data
        ...
        setTimeout(() => {
          ...
          const div = _this.$refs.map.children[0].children[0].children[0].children[2].children[2].querySelector('img[src="' + imgUrl + '"]').parentNode
  ...
  methods: {
    setMapData() {
      this.map = initMap('container-pc', this.currentPoi.center, this.config.defaultZoom)
        ...
        getPowerAround(this.map.center.lat, this.map.center.lng, dis, this, function (r) {
          ...
          if (_this.selectedResId) {
            ...
            if (m) {
              ...
              setTimeout(() => {
                let imgUrl = 'images/power/' + _this.selectedResType + 's.png?id=' + _this.selectedResId
                const div = _this.$refs.map.children[0].children[0].children[0].children[2].children[2].querySelector('img[src="' + imgUrl + '"]').parentNode
                let html = div.outerHTML
                const left = _this.getProperty(html, 'left', 0)
                const top = _this.getProperty(html, 'top', 0)
                _this.openResource(left + 30, top + 55, _this.selectedResType, _this.selectedResId)
              }, 150)
            }
          }
        })
    ...
    addRes(type, lat, lng, id, large) {
      if (type == '6') {
        ...
        if (large || !cache[id]) {
          ...
          let icon = new window.qq.maps.MarkerImage('images/power/' + iconUrl + '.png?id=' + id, size, null, null, size)
          let marker = new window.qq.maps.Marker({
            position: new window.qq.maps.LatLng(lat, lng),
            map: this.map,
            icon: icon,
    ...
    openResource(left, top, type, id) {
      ...
      if (canRight) {
        this.$refs.resPanel.setLocation(panelLeft, top - typeOffsetY - 10, offset[0], offset[1])
