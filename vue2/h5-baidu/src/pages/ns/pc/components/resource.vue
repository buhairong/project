    ...
    <div class="foot">
      <div class="search-warp">
        <div class="search-btn-wrap" @click="showPoiSearchHandler">
          <img src="https://cdn-app.nio.com/pe/ns/search.svg" class="search-icon">
          搜索
        </div>
        <div class="btn-group">
          <div class="city-search-btn" @click="showCitySearchHandler">
            <input class="currentCity" type="text" v-model="currentCity.name" disabled>
            <img src="https://cdn-app.nio.com/PE/2021/11/11/0d547883-1ec2-40f4-8cda-57b5727eb9b8.svg" class="arrow">
          </div>
          <div 
            class="filter-btn"
    ...
    >
      ...
      <div class="poi-search-wrap">
        ...
        <img 
          ...
          src="https://cdn-app.nio.com/PE/2021/11/12/95373408-d5be-4734-bb90-b7df1883b234.svg"
          @click="poiSearchText = ''"
        >
      </div>
      ...
      <div class="cotent-wrap">
        ...
        <div class="pois" v-show="pois.length">
          ...
          >
            <div class="poi-icon">
              <img src="https://cdn-app.nio.com/PE/2021/11/12/14140e6a-4184-4247-8599-3b99f3f858f8.svg">
            </div>
            <div class="poi-info">
    ...
    <div class="city-search-warp" v-show="showCitySearch">
      ...
      <div class="tabs">
        ...
        <div class="sel-province-arrow" v-show="cityList.length">
          <img src="https://cdn-app.nio.com/PE/2021/11/15/0bbf7c98-3fe4-468a-9bdd-437abb5b833f.svg">
        </div>
        <div 
          class="tab city"
      ...
      <div class="content-wrap">
        <div class="province-wrap" v-show="activeSelTab === 'province'">
          <div 
            class="province-item"
            :class="{'touch-province': selProvince === province.name,
...
export default {
  ...
  methods: {
    ...
    showMarkers(markerPoint, sizeW = 16, sizeH = 17, isBig = false) {
      let isRange = false
      let icon = 'https://cdn-app.nio.com/pe/ns/swap.svg'
      ...
      if (isBig) {
        ...
        if (markerPoint.type === 'PowerSwap') {
          icon = 'https://cdn-app.nio.com/ns/ns/swapafter.svg'
        } else if (markerPoint.type === 'ChargeStation' && markerPoint.charge_station_type === 'NioChargeStationNpc') {
          icon = 'https://cdn-app.nio.com/pe/ns/otherpinafter.svg'
        } else if (markerPoint.type === 'ChargeStation' && markerPoint.charge_station_type === 'NioChargeStationDest') {
          icon = 'https://cdn-app.nio.com/pe/ns/otherpinafter.svg'
        } else if (markerPoint.type === 'ChargeStation' && markerPoint.charge_station_type === 'ThirdChargeStation') {
          icon = 'https://cdn-app.nio.com/pe/ns/otherpinafter.svg'
        }
      } else {
        if (markerPoint.type === 'PowerSwap') {
          icon = 'https://cdn-app.nio.com/pe/ns/swappin.svg'
          if (markerPoint.construction !== '11') {
            isRange = true
          }
          zIndex = 15
        } else if (markerPoint.type === 'ChargeStation' && markerPoint.charge_station_type === 'NioChargeStationNpc') {
          icon = 'https://cdn-app.nio.com/pe/ns/otherpin.svg'
          zIndex = 14
        } else if (markerPoint.type === 'ChargeStation' && markerPoint.charge_station_type === 'NioChargeStationDest') {
          icon = 'https://cdn-app.nio.com/pe/ns/otherpin.svg'
          zIndex = 13
      ...
      const myIcon = new window.BMap.Icon("https://cdn-app.nio.com/pe/ns/smallswap.svg", size)
    ...
    openResource(left, top) {
      ...
      if (canRight) {
        this.$refs.resPanel.setLocation(left + 65, top - 50, offset[0], offset[1])
        this.resPanel.left = true
        this.resPanel.right = false
      } else {
        this.$refs.resPanel.setLocation(left - 364 - 75, top - 50, offset[0], offset[1])
    ...
    getMapMovingOffset() {
      const style = this.$refs.resource.children[0].children[0].children[0].style
    ...
    selectedToSmall() {
      ...
      this.selected.setIcon(new window.qq.maps.MarkerImage(url, size, null, anchorb, size))
    },
    ...
    setMyMarker() {
      ...
      const myIcon = new window.BMap.Icon("https://cdn-app.nio.com/PE/2021/11/16/f59dd903-0e92-4b54-8e8e-e274644ce41e.svg", size)
      const point = new window.BMap.Point(this.curPosition.lng, this.curPosition.lat)
    ...
    onPoiSearch() {
      ...
      getPoiSuggest(this, (result) => {
        const pois = result.pois
        if (pois && pois.length) {
          this.pois = result.pois.map(item => {
            const re =new RegExp(this.poiSearchText, 'g')
            item.name = item.name.replace(re, `<span style="color:#040B29;">${this.poiSearchText}</span>`)
            item.address = item.address.replace(re, `<span style="color:#040B29;">${this.poiSearchText}</span>`)
            return item
          })
        } else {
...
.poi-warp .header, .city-search-warp .header{
  height: 0.28rem;
  display: flex;
  justify-content: space-between;
  align-items: center;
...
.poi-warp .header .title, .city-search-warp .header .title {
  color: #040B29;
  font-size: 0.18rem;
  font-weight: 500;
}
...
.content-wrap .province-item, .content-wrap .city-item {
  height: 0.4rem;
  display: flex;
  align-items: center;
  font-size: 0.14rem;
