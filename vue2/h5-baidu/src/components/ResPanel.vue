<template>
  ...
  >
    ...
    <div style="position: relative">
      ...
      <div class="title">
        <span v-show="data.type == 5">会客点：</span>{{ data.title }}
      </div>
      ...
      <div v-if="data.type == 1">
        <div class="resource" style="height: 50px; line-height: 50px">
          <img
            class="icon -invisible -internal"
            src="images/res/battery.png"
            width="16"
            height="16"
          />
          <div class="unit">
            可用电池<span class="num">{{ data.valid }}</span
            >块
          </div>
          <div class="unit">
            电池总数<span class="num">{{ data.battery }}</span
            >块
          </div>
        </div>
        <div class="line"></div>
        ...
        <div class="resource">
          <img
            class="icon-free -invisible -internal"
            src="images/res/free.png"
            width="16"
            height="16"
          />
      ...
      <div v-if="data.type == 2 || data.type == 3 || data.type == 4">
        <div class="resource" style="height: 50px; line-height: 50px">
          <img
            class="icon -invisible -internal"
            src="images/res/charge.png"
            width="16"
            height="16"
          />
          <div v-if="data.dc" class="unit">
            快充总数<span class="num">{{ data.dc }}</span
            >根
          </div>
          <div v-if="data.ac" class="unit">
            慢充总数<span class="num">{{ data.ac }}</span
            >根
          </div>
        </div>
      </div>
      <div v-if="data.type == 5">
        <div class="desc">
          <div>
            当站只建设在单向服务区反向就用会客点表示，用户到会客点后请联系专员，由专员开车给用户
          </div>
        </div>
        <div style="height: 10px"></div>
        <div class="line"></div>
