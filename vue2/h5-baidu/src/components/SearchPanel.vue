  ...
  <div id="searchPanel">
    <div v-show="display == 1">
      <div style="padding: 10px; position: relative">
        <div class="search-bar">
          <img class="-invisible -internal" src="images/search.png" />
          <input
            @input="inputText"
            type="text"
            placeholder="查找地点、资源"
          ...
          />
          <img
            class="clear -invisible -internal"
            @click="clear"
      ...
      <div class="overflow-scroll" ref="overflow3" v-loading="loading">
        <div v-if="searchText">
          <div v-if="!hasResult">
            ...
            <div style="text-align: center">
              <img
                ...
                src="images/search/no-result.png"
                width="320px"
                height="160px"
              />
            </div>
          ...
          <div v-else>
            <div class="section">
              {{ poiResult.length == 0 ? "暂无相关目的地" : "目的地" }}
            </div>
            <div class="result" ref="poiResult">
              <div
                @click="showPoi(c)"
                v-for="(c, i) in poiResult"
                :key="'p' + i"
              >
            ...
            <div
              v-if="!isPc && poiResult.length > 3 && resResult.length > 0"
              @click="showAllPoi"
            >
            ...
            <div class="section">
              {{ resResult.length == 0 ? "暂无相关资源" : "资源" }}
            </div>
            <div class="result" ref="resResult">
              <div
                @click="showRes(c)"
                v-for="(c, i) in resResult"
                :key="'r' + i"
              >
            ...
            <div
              v-if="!isPc && resResult.length > 3 && poiResult.length > 0"
              @click="showAllRes"
            >
    ...
    <div v-show="display == 2">
      ...
      <div class="overflow-scroll" ref="overflow">
        ...
        <div id="city-current">
          <img
            class="-invisible -internal"
            src="images/search/location-selected.png"
            width="24"
          />
        ...
        <div class="city-list">
          <div @click="clickCity(c)" v-for="(c, i) in cityList" :key="i">
            {{ c.name }}
          </div>
        </div>
      </div>
    ...
    <div v-show="display == 3">
      ...
      <div class="overflow-scroll city-list" ref="overflow2">
        <div
          @click="clickCity(c)"
          v-for="(c, i) in citySelectedChildren"
          :key="i"
        >
          {{ c.name }}
        </div>
  ...
  methods: {
    request(code, keyword) {
      ...
      suggest(code, keyword, this, function (data) {
        ...
        _this.$nextTick(() => {
          if (_this.isPc) {
            _this.$refs.poiResult.style.height =
              _this.unitHeight * _this.poiResult.length + "px";
            _this.$refs.resResult.style.height =
              _this.unitHeight * _this.resResult.length + "px";
          } else {
            let countP =
              _this.poiResult.length > 3 ? 3 : _this.poiResult.length;
            if (_this.resResult.length == 0) {
              countP = _this.poiResult.length;
            }
            let countR =
              _this.resResult.length > 3 ? 3 : _this.resResult.length;
            if (_this.poiResult.length == 0) {
              countR = _this.resResult.length;
            }
            console.log(countP, countR);
            _this.$refs.poiResult.style.height =
              _this.unitHeight * countP + "px";
            _this.$refs.resResult.style.height =
              _this.unitHeight * countR + "px";
          }
          _this.loading = false;
        });
      });
    ...
    showAllPoi() {
      ...
      if (this.poiShowAll) {
        this.$refs.poiResult.style.height =
          this.unitHeight * this.poiResult.length + "px";
      } else {
        this.$refs.poiResult.style.height =
          this.unitHeight *
            (this.poiResult.length > 3 ? 3 : this.poiResult.length) +
          "px";
      }
    },
    showAllRes() {
      ...
      if (this.resShowAll) {
        this.$refs.resResult.style.height =
          this.unitHeight * this.resResult.length + "px";
      } else {
        this.$refs.resResult.style.height =
          this.unitHeight *
            (this.resResult.length > 3 ? 3 : this.resResult.length) +
          "px";
      }
    },
    showPoi(c) {
    ...
    saveHistory() {
      cookie.set(COOKIE_HISTORY_NAME, JSON.stringify(this.history), {
        expires: 7,
      });
    },
  },
