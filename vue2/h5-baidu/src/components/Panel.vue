<template>
  <div class="panel" ref="panel" v-loading="panel.loading">
    <div v-if="panel.showCollapse" style="height: 20px; text-align: center" @click="toggle">
      <div class="toggle">
        <img class="nio-invisible nio-internal" :src="panel.collapsed?'images/panel/toggle-e.png':'images/panel/toggle-c.png'" width="26px" height="8px">
      </div>
    </div>
    <div v-else style="height: 20px"></div>
    <div @touchstart="startMove" @touchmove="moving" @touchend="stopMove">
      <div class="trans-panel" style="height: 285px; overflow: hidden; position: relative" ref="poiPanel">
        <div v-if="panel.showSearch" @click="showSearchPanel" class="search">
          <img class="nio-invisible nio-internal" src="images/search.png" width="24px" height="24px">
          <span>查找地点、加电资源</span>
        </div>
        <div @click="closePanel" v-if="panel.showClose" style="position: absolute; top:0; right: 0">
          <img class="nio-invisible nio-internal" src="images/close.png" width="30">
        </div>
        <div v-if="panel.showSearch" style="height: 15px"></div>
        <div class="title" :style="{'width': panel.showClose? '275px':''}">{{ panel.title }}</div>
        <div v-if="panel.showDistance" class="distance" @click="panel.showDistancePanel=true">
    ...
    <el-drawer :modal-append-to-body="false" ref="drawDistance" :visible.sync="panel.showDistancePanel" title="距离范围"
               direction="btt" :size=size :show-close="false">
      <div @click="closeDistance" class="close"><img class="nio-invisible nio-internal" src="images/close.png" width="30px" height="30px"></div>
      <div style="padding-left: 25px; padding-right: 25px">
        <div class="description">附近{{ panel.distancePre }}km资源</div>
        <div>
          <el-slider v-model="panel.distancePre" :show-tooltip="false" :min=1 :max=20></el-slider>
        </div>
        <div class="marker">
          <div>1km</div>
          <div>20km</div>
